...
def test_signal_strength_sizes_orders_symmetrically_with_caps():
    ...
    assert order_size({'side': 'BUY', 'difference_hz': 11}) == D('13')
    assert order_size({'side': 'BUY', 'difference_hz': 20}) == D('25')
    assert order_size({'side': 'SELL', 'difference_hz': -11}) == D('13')
    assert order_size({'side': 'SELL', 'difference_hz': -200}) == D('25')
    import pytest
...
def test_variable_orders_report_trade_value_separately_from_costs():
    ...
    large = a.execute('BUY', D('50'), D('25'))
    assert a.cash == D('74')
    assert large['shares'] > small['shares']
    assert 24 < large['trade_value'] < 25
    assert large['cost'] < .03
    assert large['cash_change'] == -25
    assert large['requested_value'] == 25
    sell = a.execute('SELL', D('50'), D('25'))
    assert sell['trade_value'] == 25
    assert sell['cash_change'] > 24
    for _ in range(10):
        a.execute('BUY', D('50'), D('25'))
    assert a.cash >= 0
    for _ in range(10):
        a.execute('SELL', D('50'), D('25'))
    assert a.shares == 0
    import pytest
    for size in ('-1', '26', 'NaN', 'Infinity'):
        with pytest.raises(ValueError):
...
def test_replay_fills_at_next_open_and_future_bars_cannot_change_input():
    ...
    assert first.account.cash == 100
    assert not first.trades
    assert first.pending_size == D('13')
    ...
    assert c2.durations[-1] == 200
    assert second.trades[0]['requested_value'] == 13
    assert second.account.cash == 87
    assert second.last['food_ms'] == 200
    second.step()
    assert second.done
    ...
    second.reset()
    assert second.account.cash == 100
...
def test_invalid_prices_and_no_shorting():
    ...
    assert a.execute('SELL', D('50')) is None
    assert a.shares == 0 and a.cash == 100
